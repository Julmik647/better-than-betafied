            ...
            system.runTimeout(() => {
                try {
                    ...
                    if (isWaterLocation(player.dimension, coords.x, coords.z)) {
                        if (attempt < 10) {
                            console.warn(`[keirazelle] Water at ${coords.x}, ${coords.z} - retry ${attempt}`);
                    ...
                    if (groundY === -1 && attempt < 10) {
                        console.warn(`[keirazelle] No ground at ${coords.x}, ${coords.z} - retry ${attempt}`);
                        coords = getRandomCoords();
                        trySpawn();
                        return;
                    }
                    ...
                    console.warn(`[keirazelle] Spawn: ${coords.x}, ${finalY}, ${coords.z} (attempt ${attempt})`);
                } catch (e) {
                    console.warn("[keirazelle] Spawn error: " + e);
                }
